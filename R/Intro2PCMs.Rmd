...
We can simulate data using Brownian Motion to see what it looks like. It's sometimes called "the Drunkard's walk". It is the process that aligns with Darwinian gradualism: Random changes in the phenotype that accumulate among lineages, resulting in similar species apppearing more similar to each other on average. It is our benchmark for the concept of "phylogenetic signal". 
```{r}
simdat <- fastBM(td2$phy, a = 0, sig2=1, internal=TRUE) # `a` is the ancestral state at the root. sig2 is the evolutionary rate.
phenogram(td2$phy, simdat, ftype='off')
```

Compare that to a tree where only 25% the variation among species comes from Brownian Motion, and the other 75% comes from random, non-phylogenetic noise. These datasets have a quarter of the original the phylogenetic signal. 
```{r}
simdat2 <- fastBM(lambdaTree(td2$phy,0.25), a = 0, sig2=1, internal=TRUE) # `a` is the ancestral state at the root. sig2 is the evolutionary rate.
phenogram(td2$phy, simdat2, ftype='off')
```
...
```{r}
#synth_tree <- get_dated_otol_induced_subtree(ott_ids=ottTable$ott_id) #this didn't work today when I ran it because the services are down. This returns a dated phylogeny
synth_tree <- get_otol_synthetic_tree(ott_ids = ottTable$ott_id) #this one works, but doesn't provide valid dates.
```

```{r}
plot(synth_tree, cex=0.75)
axisPhylo() #if you used the second line, you'll see the dates are wrong.
```
